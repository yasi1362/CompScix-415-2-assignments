...
url_bike <- paste0("https://api.jcdecaux.com/vls/v1/stations?contract=",'Dublin',"&apiKey=b320f0b8ee7497f3b9238c637ac31bbfb27a8388")
df_api_bike <- fromJSON(url_bike)
...
bike_map <- url_bike
geocode(bike_map)
bike_map_geo <- geocode(bike_map)

map_it <- leaflet() %>%
  addTiles() %>%
  addMarkers(lng = ~lon, lat = ~lat, data = bike_map_geo)
map_it
```


```{r}
map_it_data <- GET_https://api.jcdecaux.com/vls/v1/stations?contract={contract_name}&apiKey={b320f0b8ee7497f3b9238c637ac31bbfb27a8388}
```
