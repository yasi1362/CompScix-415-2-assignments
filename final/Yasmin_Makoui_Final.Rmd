...
```{r}
#import csv file
file_path_ab <- '/Users/yasminmakoui/downloads/ab_test_data.csv'
ab_data <- read_csv(file = file_path_ab)

ab_data_tibble <- as_tibble(ab_data)

ab_table <- ab_data_tibble %>% group_by(version, conversion) %>%
  summarize(num_A = sum(conversion))
...
conv_prop_a <- ab_table_total %>% summarise(total_count = conv_count)
